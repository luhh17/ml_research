...
class RNN(nn.Module):
    def __init__(self, args):
        ...
        self.rnn_encoder = nn.RNN(input_size=args['embed_dim'], hidden_size=args['embed_dim'], num_layers=args['num_enc_layers'], batch_first=True)
...
class LSTM(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.embedding = nn.Linear(args['char_dim'], args['embed_dim'])
        self.lstm_encoder = nn.LSTM(input_size=args['embed_dim'], hidden_size=args['embed_dim'], num_layers=args['num_enc_layers'], batch_first=True)
        self.out = nn.Linear(args['embed_dim'], 1)
