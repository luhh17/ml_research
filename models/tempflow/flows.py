...
class FlowSequential(nn.Sequential):
    ...
    def forward(self, x, cond):
        ...
        for module in self:
            x, log_abs_det_jacobian = module(x, cond)
...
class LinearMaskedCoupling(nn.Module):
    def __init__(self, input_size, hidden_size, n_hidden, mask, cond_label_size):
        super().__init__()
        # scale function
        self.register_buffer("mask", mask)
        s_net = [
            nn.Linear(
                input_size + (cond_label_size if cond_label_size else 0),
                hidden_size
            )
        ]
    ...
    def forward(self, x, cond_y=None):
        ...
        s = self.s_net(mx if cond_y is None else torch.cat([cond_y, mx], dim=-1))
        t = self.t_net(mx if cond_y is None else torch.cat([cond_y, mx], dim=-1)) * (
                1 - self.mask
        )
    ...
    def inverse(self, u, cond_y=None):
        ...
        s = self.s_net(mu if cond_y is None else torch.cat([cond_y, mu], dim=-1))
        t = self.t_net(mu if cond_y is None else torch.cat([cond_y, mu], dim=-1)) * (
                1 - self.mask
        )
...
class Flow(nn.Module):
    ...
    def log_prob(self, x, cond, mask):
        u, sum_log_abs_det_jacobians = self.forward(x, cond)
        return torch.sum((self.base_dist.log_prob(u) + sum_log_abs_det_jacobians) * mask, dim=-1)
